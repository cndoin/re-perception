...
- 路径用正斜杠，含空格必须加引号
- 自检：`python selftest.py`（97 个用例，期望全绿）
...
| `rules/*.yml` | **能力规则库**（capa 格式）：4 个文件共 20 条，跨 7 个 capa 命名空间（进程/加密/网络/反分析/加载/发现/持久化），带 ATT&CK 映射 |
| `scripts/selftest.py` | 自检套件（正确性 + 稳定性 + 性能，共 97 个用例，含 74 条 x86 + 18 条 ARM64 黄金指令向量） |
