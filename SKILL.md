...
- 路径用正斜杠，含空格必须加引号
- 自检：`python selftest.py`（81 个用例，期望全绿）
...
| `scripts/_quirk.py` | dbghelp 已知缺陷归一化表（**生产模块**，勿挪进 `_dev/`） |
| `rules/*.yml` | **能力规则库**（capa 格式）：进程/加密/网络/反分析四类共 20 条，带 ATT&CK 映射 |
| `scripts/selftest.py` | 自检套件（正确性 + 稳定性 + 性能，共 81 个用例，含 74 条 x86 + 18 条 ARM64 黄金指令向量） |
...
| `_` 模块被挪进 `_dev` | `_quirk.py` 曾是生产模块，被误当脚手架挪走 —— 全靠这条拦住 |
