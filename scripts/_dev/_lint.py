...
def check_underscore_guard():
    ...
    这条护栏的**真实目的**：`_` 前缀文件易被误当脚手架挪进 _dev/，
    而 `_dev/` 不随发布分发，结果发布物里少一个必需文件。
    `_quirk.py` 就栽过一次（它是用户要读的 dbghelp 缺陷清单）。
    所以要拦的是「**发布物必需的模块**被挪走」—— 判据是「有没有被生产
    代码 import」，那是当前能静态判定的最强近似。
